...
| `document/facts.rs` | Shared complex-source feature inventory, dense source-evidence masks, propagated semantic facts, and cleanup-safe source facts |
...
- **Cache semantic source evidence.** Run a tiny source gate before taking a retained-fragment snapshot. Build feature-specific callout, footnote, math, accessible-math, and data-table evidence as sparse node sets only when the gate finds that feature. Resolve fragment references against only their target IDs. Pass the cached evidence through hard cleanup, heuristic cleanup, final cleanup, and semantic compilation. Use cheap tag and attribute gates before rich recognizers.
